...
def preprocess(dataset, test_and_split=False, test_size=0.2):
    ...
    #features=dataset.loc[:, 'league':'away_odds_n']
    #features=dataset
    #swap_columns(features)
    #features=features.loc[:,'league':'away_odds_n']
    features=dataset.loc[:,'home_team_rank':'away_odds_n']
    ...
    if test_and_split:
       x_train, x_test, y_train, y_test = train_test_split(features, target,test_size=test_size, random_state=2,shuffle=True)
